...
-->
在阅读这篇文档之前，强烈建议您先熟悉一下[Kubernetes日志概况](/docs/concepts/cluster-administration/logging)
...
{{< note >}}
一般地，Stackdriver logging仅收集容器的标准输出和标准错误流。
为了收集您的应用程序写入一个文件（例如）的任何日志，请参见Kubernetes日志概述中的[sidecar方式](/docs/concepts/cluster-administration/logging#sidecar-container-with-a-logging-agent)
{{< /note >}}
...
-->
为了接收日志，您必须将Stackdriver Logging agent 部署到集群中的每个节点。
这个agent是一个配置的`fluentd`，它的配置存在一个`ConfigMap`中，且实例使用Kubernetes的`DaemonSet`进行管理。
实际的`ConfigMap` and `DaemonSet`部署，取决您的个人集群设置。
...
-->
如果您的集群是部署在Google Kubernetes Engine，那么 Stackdriver 是默认的日志解决方案。
Stackdriver Logging会默认部署到您的新集群上，除非您明确地不选择。
...
-->
为了将 Stackdriver Logging 部署到您正在使用 `kube-up.sh` 创建的新集群上，执行如下操作：
...
-->
1. 设置环境变量`KUBE_LOGGING_DESTINATION`到`gcp`
1. **如果您不是跑在GCE上**，在`KUBE_NODE_LABELS`变量种引入`beta.kubernetes.io/fluentd-ds-ready=true`
...
集群启动后，每个节点都应该运行 Stackdriver Logging agent。
`DaemonSet` and `ConfigMap`作为附加组件进行配置。
如果您不是使用`kube-up.sh`，考虑不使用预先配置的日志方案启动群集，然后部署 Stackdriver Logging agents 到正在运行的群集。
...
{{< warning >}}
除了Google Kubernetes Engine，Stackdriver logging 守护进程在其他的平台有已知的问题。
请自行承担风险。
{{< /warning >}}
